        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting && !isAnimated) {
                    ...
                    // Анимируем карточки после заголовка
                    const headerAnimationDuration = 600;
                    cards.forEach((card, index) => {
                        setTimeout(() => {
                            card.classList.add('animate');
                        }, headerAnimationDuration + (index * 200));
                    });

                    // Анимируем пагинацию после появления всех карточек
                    const totalCardsDelay = headerAnimationDuration + (cards.length * 200);
                    setTimeout(() => {
                        if (pagination) pagination.classList.add('animate');
                    }, totalCardsDelay - 1000);
                }
            });
        }, {
            threshold: 0.2,
...
// Удаляем старый TestimonialsAnimationModule и добавляем единую инициализацию
...
const HeroAnimationModule = {
    init() {
        ...
        // Разбиваем текст заголовка на слова, а ��атем на б��квы для анимации
...
const ApplicationsAnimationModule = {
    init() {
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    if (entry.target === applicationsSection) {
                        // Анимируем заголовок
                        if (title) title.classList.add('animate');
                        
                        // Анимируем карточки с задержкой после заголовка
                        const headerAnimationDuration = 500; // 1 секунда для анимации заголовка
                        
                        cards.forEach((card, index) => {
                            setTimeout(() => {
                                card.classList.add('animate');
                            }, headerAnimationDuration + (index * 150)); // Задержка после заголовка + интервал между карточками
                        });
                    }
                }
            });
        ...
        // Наблюдаем за всей секцией вместо отдельных элементов
            ...
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    if (entry.target === productsSection) {
                        ...
                        // Анимируем карточки с задержкой после заголовка
                        const headerAnimationDuration = 500;
                        
                        cards.forEach((card, index) => {
                            setTimeout(() => {
                                card.classList.add('animate');
                            }, headerAnimationDuration + (index * 150));
                        });
                    }
                }
            });
...
const AdvantagesAnimationModule = {
    init() {
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    if (entry.target === advantagesSection) {
                        // Анимируем заголовок
                        if (title) title.classList.add('animate');
                        
                        // Анимируем карточки последовательно
                        cards.forEach((card, index) => {
                            setTimeout(() => {
                                card.classList.add('animate');
                            }, 500 + (index * 200)); // 800ms для заголовка + интервал между карточками
                        });
                    }
                }
            });
...
// const TestimonialsAnimationModule = {
//     init() {
//         const testimonialsSection = document.querySelector('.testimonials');
//         if (!testimonialsSection) return;
...
//                     // Анимируем заголовок (0ms)
//                     if (title) title.classList.add('animate');
                    
//                     // Анимируем карточки после заголовка (800ms)
//                     const headerAnimationDuration = 600;
//                     cards.forEach((card, index) => {
//                         setTimeout(() => {
//                             card.classList.add('animate');
...
//                     setTimeout(() => {
//                         if (pagination) pagination.classList.add('animate');
//                     }, totalCardsDelay - 1200); // Дополнительные 200мс после последней карточки
//                 }
//             });
//         }, {
//             threshold: 0.2,
...
// // Добавляем инициализацию
// document.addEventListener('DOMContentLoaded', () => {
//     TestimonialsAnimationModule.init();
// });
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting && !isAnimated) {
                    ...
                    // Анимируем карточки новостей с задержкой
                    const headerAnimationDuration = 600;
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting && !isAnimated) {
                    ...
                    // Анимируем партнеров с задержкой
                    partners.forEach((partner, index) => {
                        setTimeout(() => {
                            partner.classList.add('animate');
                        }, 600 + (index * 100)); // Меньший интервал для более динамичного появления
                    });
                }
            });
        }, {
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting && !isAnimated) {
                    ...
                    // Анимируем элементы FAQ последовательно
